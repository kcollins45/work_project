...
int main(){
    ...
    printf("\t-----------------------------------\n");
    printf("\t-   Part 2: Running a Tournament  -\n");
    printf("\t-----------------------------------\n");
    printf("\n\t=========Test #4: Ensure number of teams is a power of 2===========\n\n");
    winner = tournament(league, 20);
    assert(winner == NULL);
    printf("\n\t\t....Test Passed\n");

    printf("\n\t=========Test #5: Should result in a single winner===========\n\n");
    ...
    printf("\n\t=========Test #6: Should result in a random winner===========\n\n");
    ...
    printf("\n\t=========Test #7: Delete team method should clean up memory for each team ===========\n\n");
    //free team array.
    for(int counter = 0; counter < NUM_TEAMS; counter++){
...
Player * draftPlayers(char * filename, int team, int num_players){
    ...
    Player * plist = malloc(sizeof(Player) * NUM_PLAYERS);
    if(fptr){
        char buff[255];
        for(int i = 0; i < num_players && fgets(buff,255, fptr);){
            //fprintf(stderr, "buff:%s", buff);
            int t = atoi(strtok(buff, ","));
            if(team == t){
                int number = atoi(strtok(NULL, ","));
                char * first = strtok(NULL, ",");
                char * last = strtok(NULL, ",");
                int off = atoi(strtok(NULL, ","));
                int def = atoi(strtok(NULL, "\n"));
                Player p = {.offensive=off, .defensive=def, .team=t, .number=number};
                strcpy(p.first, first);
                strcpy(p.last, last);
                plist[i] = p;
                //fprintf(stderr, "%d: %s %s, %d, %d\n", plist[i].number,plist[i].first, plist[i].last, plist[i].offensive, plist[i].defensive);
                i++;
            }
        }
        fclose(fptr);
